...
def serialize_site_about() -> dict:
    ...
    profile_bio = ''
    if author:
        data = serialize_user(author)
        display_name = data['displayName']
        avatar = data['avatar']
        profile_bio = data['bio']
    body = about.body or profile_bio
    return {
        'displayName': display_name,
        'avatar': avatar,
        'tagline': about.tagline or '',
        'quote': about.quote or '',
        'body': body,
        'focusTags': about.focus_tags or [],
        'stats': about.stats or [],
        'timeline': about.timeline or [],
        'timelineSubtitle': about.timeline_subtitle or '',
        'influences': about.influences or [],
        'techStack': about.tech_stack or [],
        'stackNote': about.stack_note or '',
    }


class SiteAboutUpdateSerializer(serializers.Serializer):
    tagline = serializers.CharField(required=False, allow_blank=True, max_length=128)
    quote = serializers.CharField(required=False, allow_blank=True)
    body = serializers.CharField(required=False, allow_blank=True)
    focusTags = serializers.ListField(
        child=serializers.CharField(max_length=64),
        required=False,
    )
    stats = serializers.ListField(child=serializers.DictField(), required=False)
    timeline = serializers.ListField(child=serializers.DictField(), required=False)
    timelineSubtitle = serializers.CharField(required=False, allow_blank=True, max_length=128)
    influences = serializers.ListField(child=serializers.DictField(), required=False)
    techStack = serializers.ListField(child=serializers.DictField(), required=False)
    stackNote = serializers.CharField(required=False, allow_blank=True)
    ...
    def update(self, instance, validated_data):
        field_map = {
            ...
            'stats': 'stats',
            'timeline': 'timeline',
            'timelineSubtitle': 'timeline_subtitle',
        ...
        for api_key, model_key in field_map.items():
            if api_key in validated_data:
                setattr(instance, model_key, validated_data[api_key])
        instance.save()
        return instance
